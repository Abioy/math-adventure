// 音效系统 — 双引擎：Web Audio API + HTML5 Audio 降级
const AudioCtx = window.AudioContext || window.webkitAudioContext;
let audioCtx;
let audioFallback = null; // HTML5 Audio fallback
...
// 尝试用 Web Audio 播放，失败则用 HTML5 Audio
function playSound(type) {
  ...
  try {
    ...
    // Web Audio 失败，用 HTML5 Audio 降级
    playFallback(type);
  }
}

// HTML5 Audio fallback — 440Hz 蜂鸣
...
function playFallback(type) {
  if (!audioFallback) {
    audioFallback = new Audio(FALLBACK_WAV);
  }
  try {
    audioFallback.pause();
